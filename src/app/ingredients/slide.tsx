...
const Gallery = () => {
  return (
    <div className="h-screen flex flex-col items-center text-black mt-50 ">
      <div className="max-w-5xl w-full min-h-[500px]">
        <h1 className="proNormal text-5xl text-center mb-30">ส่วนผสมใหม่</h1>
        <Swiper
          effect={"coverflow"}
          grabCursor={true}
          centeredSlides={true}
              ...
              {({ isActive, isPrev, isNext }) => (
                <div className={`flex justify-center transition-all duration-500 ${isActive || isPrev || isNext ? "opacity-100" : "opacity-0"}`}>
                  <div className={`w-72 flex flex-col items-center justify-center rounded-lg shadow-lg transition-all duration-500 ease-in-out transform bg-white p-4 ${
                    isActive ? "w-26 md:w-70 h-96 scale-110 " : "h-40 scale-90 overflow-hidden translate-y-30"
                  }`}>
